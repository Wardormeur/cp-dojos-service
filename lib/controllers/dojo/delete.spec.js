...
lab.experiment('delete a dojo and its relationships', function () {
  ...
  lab.before(function (done) {
    ...
    seneca.act({role: plugin, ctrl: 'lead', cmd: 'save', lead: leadStub, user: user}, function (err, savedLead) {
      ...
      seneca.act({role: plugin, ctrl: 'lead', cmd: 'submit', lead: savedLead, user: user}, function (err, submittedLead) {
        ...
        mockSeneca({ role: 'cd-dojos', ctrl: 'dojo_public_fields', cmd: 'load', id: submittedLead.application.dojo.id },
          function (args, mockDone) {
            seneca.act({ role: 'cd-dojos', entity: 'dojo', cmd: 'load', id: args.id }, mockDone);
        });
  ...
  lab.test('the dojo should be soft deleted', function (done) {
    seneca.act({role: 'cd-dojos', entity: 'dojo', cmd: 'load', id: dojo.id}, function (err, deletedDojo) {
